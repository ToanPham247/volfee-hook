...
contract VolAdaptiveFeeTest is Test, Deployers {
    ...
    function _deployHook() internal returns (VolFeeHook h) {
        ...
        h = new VolFeeHook{salt: salt}(IPoolManager(address(manager)), uint16(300), currency0, PROJECT, INITIAL_LP_FEE, ALPHA_BPS, K_NUM, K_DEN, MIN_LP_FEE, MAX_LP_FEE, expectedToken, SQRT_PRICE_1_1, TS);
        require(address(h) == addr, "hook miner mismatch");
    }

    function _deployHookWithParams(uint16 _feeTotalBps, uint24 _initialLpFee, uint16 _alphaBps, uint256 _kNum, uint256 _kDen, uint24 _minLpFee, uint24 _maxLpFee) internal returns (VolFeeHook h) {
        address expectedToken = Currency.unwrap(currency1);
        bytes memory args = abi.encode(address(manager), _feeTotalBps, currency0, PROJECT, _initialLpFee, _alphaBps, _kNum, _kDen, _minLpFee, _maxLpFee, expectedToken, SQRT_PRICE_1_1, TS);
        (address addr, bytes32 salt) = HookMiner.find(address(this), FLAGS, type(VolFeeHook).creationCode, args);
        h = new VolFeeHook{salt: salt}(IPoolManager(address(manager)), _feeTotalBps, currency0, PROJECT, _initialLpFee, _alphaBps, _kNum, _kDen, _minLpFee, _maxLpFee, expectedToken, SQRT_PRICE_1_1, TS);
        require(address(h) == addr, "hook miner mismatch");
    }
    ...
    function _hookAndPool() internal returns (VolFeeHook h, PoolKey memory key, PoolId id) {
        h = _deployHook();
        (key, id) = _initPool(h);
    }
    ...
    function test_fee_clamps_at_maxLpFee() public {
        VolFeeHook h = _deployHookWithParams(300, 3000, 8000, 10000, 1, 500, 100000);
